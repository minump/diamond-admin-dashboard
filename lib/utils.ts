...
export function fetchSessionData() {
  ...
  const primaryIdentity = document.cookie.split('; ').find(row => row.startsWith('primary_identity='))?.split('=')[1] ?? '';
  const name = document.cookie.split('; ').find(row => row.startsWith('name='))?.split('=')[1] ?? '';
  const email = document.cookie.split('; ').find(row => row.startsWith('email='))?.split('=')[1] ?? '';
  const institution = document.cookie.split('; ').find(row => row.startsWith('institution='))?.split('=')[1] ?? '';
